...
async def search_vk(
    ...
    if not token:
        return {"status": "error", "message": "VK token is missing", "items": []}
    ...
    try:
        ...
        return {"status": "error", "message": f"VK request failed: {e}", "items": []}

    if data.get("error"):
        return {
            "status": "error",
            "message": data["error"].get("error_msg", "VK search failed"),
            "vk_error": data["error"],
            "items": [],
        }
